...
//   private calculateDistance(pos1: Position, pos2: Position): number {
//     // console.log(Math.abs(pos1.x - pos2.x) + Math.abs(pos1.y - pos2.y));
//     return Math.abs(pos1.x - pos2.x) + Math.abs(pos1.y - pos2.y);
//   }
    ...
    for (const center of this.OCCUPATION_CENTERS) {
      ...
      if (path) {
        ...
        if (path) {
          ...
          const startDistance = this.calculateDistance(myPos, center);
          const endPos = path[path.length - 1];
          const endDistance = this.calculateDistance(endPos, center);
